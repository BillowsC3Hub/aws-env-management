...
class Input(BaseModel):
    Messages: list
    destination_uris: Optional[list]
    ...
    # Uses same RegEx expression as the AWS S3_BUCKET environment variable
    @validator('destination_uris')
    def check_uris(cls, v):
        i = 0
        for m in v:
            x = v[i]
        if re.match(r"^[\w!\-.*'\(\)/&$@=;:+,? ]*$", x) == None:
            raise ValueError('Char not allowed in Destination Uris')
        i += 1
        return v
